...
이 코드는 lazy initialization 으로, 특정 객체가 실제로 필요할 때까지 초기화를 미루다가, 동시에 단 한번만 초기화하여 값을 반환하는 코드이다.
...
또한 복잡하게 오래 걸리는 작업, I/O, 네트워크 작업과 같이 빨리 끝나지 않는 작업에는 가급적이면 락을 잡지 않도록 하자. 락을 오래 잡으면 성능 문제를 야기할 수 있다.
